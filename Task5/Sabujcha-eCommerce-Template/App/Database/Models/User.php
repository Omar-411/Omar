...
class User extends Connection implements Crud
{
    private $id, $first_name, $last_name, $email, $phone,
        $password, $gender, $status, $verification_code,
        $admin_id, $email_verified_at, $created_at, $updated_at;
   ...
   //  User CRUD
    ...
    public function create()
    {
        $stmt = $this->con->
        prepare(
        "INSERT INTO `users`( `first_name`, `last_name`, `email`, `phone`,`password`, `gender`,`verification_code`)
        VALUES( ?,?,?,?,?,?,?)");
        if (!$stmt) {
            return false;
        }
        ...
        return $stmt->execute();   // "Excute()" returns true or false
    }
    ...
    {
        $stmt = $this->con->prepare("UPDATE `users` SET `email_verified_at` = ? WHERE `email` = ?");
        if(!$stmt){
            return false;
        }
